...
provider "kubernetes" {
  ...
  username               = local.cluster.kube_config.0.username
  password               = local.cluster.kube_config.0.password
  client_certificate     = base64decode(local.cluster.kube_config.0.client_certificate)
  client_key             = base64decode(local.cluster.kube_config.0.client_key)
    ...
    template {
      ...
      spec {
        ...
        container {
          ...
          image             = "${var.dependency.cloudfairy_cluster.container_registry_url}/${local.env_name}/${local.service_name}:${local.docker_tag}"
